...
│
├── AGENTS.md                    # Agent configuration (Cursor and others)
├── codex.md                     # GitHub Copilot/Codex configuration (optional)
└── GEMINI.md                    # Gemini-specific configuration (optional)
```
...
Example (`openspec/config.yaml` for this project):

```yml
context: |
  Project: Reading Analytics Platform (desktop-first personal reading analytics)
...
Skills live in `ai-specs/skills/` and are mirrored into `.cursor/skills/` via relative symlinks. The agent loads a skill automatically when a request matches its description (per `AGENTS.md` §6). The most useful ones in day-to-day work are **`enrich-us`**, **`using-git-worktrees`**, **`writing-skills`**, and **`code-auditing`**:
...
- **Canonical Placement**: Prefer `ai-specs` for skills; expose through `.cursor/skills` symlinks. Use `AGENTS.md` at repo root for rules.

## 📚 Technical context

### Project-specific references (Reading Analytics Platform)

Keep these aligned with the codebase:
