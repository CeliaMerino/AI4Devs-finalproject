...
- **Feature/ticket identifiers**: e.g. ticket IDs (e.g. `KAN-9`), branch names, or short feature labels. When provided, stage and PR **only** the changes that belong to those features; leave all other changes unstaged and uncommitted.
...
- Create or update the PR for the current branch:
  - **Title**: Clear, aligned with the commit (e.g. include ticket ID if applicable: `[KAN-9] Add edition cover picker to add-book flow`).
